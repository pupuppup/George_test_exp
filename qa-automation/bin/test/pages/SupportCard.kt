...
class SupportCard(private val driver: AndroidDriver) {
    val cardSelector = By.xpath("//*[contains(@text, 'Підтримка 24/7') or contains(@text, '24/7 Support') or contains(@text, 'Podpora 24/7')]")
    val buttonSelector = By.xpath("//*[contains(@text, 'Зателефонувати') or contains(@text, 'Call') or contains(@text, 'Zavolať')]")
